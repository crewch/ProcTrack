        ...
        static async Task RunAsync()
        {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                var content = new StringContent(
                    ...
                    Encoding.UTF8,
                   "application/json");
                HttpResponseMessage response = await client.PostAsync($"http://{host}:8000/api/auth/user/create", content);
            }
            ...
            for (int i = 0 ; i < rowCount2; ++i)
            {
                ...
                var content = new StringContent(
                    System.Text.Json.JsonSerializer.Serialize(data),
                     Encoding.UTF8,
                    "application/json");
                HttpResponseMessage response = await client.PostAsync($"http://{host}:8000/api/auth/user/groups/create", content);
            }
        }
